...
define(function(require, exports, module) {
    ...
    // 获取开账时间
    var openAccountTime = (parent.SYSTEM && parent.SYSTEM.openAccountTime) ? parent.SYSTEM.openAccountTime + '-00 00:00:00' : '1900-07-01 00:00:00';
    var config = {
        path: '', //laydate所在路径
        defSkin: 'default', //初始化皮肤
        format: 'YYYY-MM-DD', //日期格式
        min: openAccountTime, //最小日期
    ...
    Dates.shde = function(elem, type) {
        ...
        // start 隐藏确认按钮 @liufei
        if (!config.isOk && elem.attributes.id.value == 'laydate_ok') {
    ...
    Dates.view = function(elem, options) {
        ...
        if (!Dates.box) {
            ...
            + Dates.viewtb
                ...
                + '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">时间</li>' + '<li><input readonly>:</li>' + '<li><input readonly>:</li>' + '<li><input readonly></li>' + '</ul>' + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a class="btn btn-common" id="laydate_clear">清空</a>' + '<a class="btn btn-common" id="laydate_today">今天</a>' + '<a id="laydate_ok" class="btn btn-common">确认</a>' + '</div>' + (config.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + '</a>' : '') + '</div>';
    ...
    Dates.events = function() {
        ...
        Dates.on(as.otoday, 'click', function() {
            var now = new Date();
            // 验证当前之间是否合法
            var thisMax = Dates.maxs;
            var thisMin = Dates.mins;
            var thisMonth = now.getMonth() < 9 ? 0 + (now.getMonth() + 1 + '') : now.getMonth() + 1 + '';
            var thisDay = now.getDate() < 9 ? 0 + (now.getDate() + '') : now.getDate() + 1 + '';
            var nowValue = (now.getFullYear() + '') + thisMonth + thisDay;
            if (thisMax.length > 3) { // 最小时间
                var minValue = thisMin[0] + thisMin[1] + thisMin[2];
                // console.log(nowValue,minValue);
                if (Number(nowValue) < Number(minValue)) {
                    Public.tips({type: 1, content : '结束时间不能小于开始时间'});
                    return;
                };
            };
            if (thisMin.length > 3) { // 最大时间
                var maxValue = thisMax[0] + thisMax[1] + thisMax[2];
                if (Number(nowValue) > Number(maxValue)) {
                    Public.tips({type: 1, content : '开始时间不能大于结束时间'});
                    return;
                };
            };
            // 如果合法执行赋值操作

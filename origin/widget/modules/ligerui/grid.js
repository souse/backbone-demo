...
(function($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [30, 45, 60], //可选择设定的每页结果数
        parms: [], //提交到服务器的参数
        columns: [], //数据源
        ...
        enabledSort: false, //是否允许排序 默认true
    ...
    //扩展一个自动下拉查询// 需要引用 autocompleter 插件
    $.ligerDefaults.Grid.editors['autocompleter'] = {
        create: function(container, editParm) {
            var column = editParm.column;
            var url = column.editor.url;
            var template = column.editor.template;
            var callback = column.editor.callback;
            var input = $('<input type="text" style="text-align:left" class="l-text-field" />');
            ...
            input.autocompleter({
                ...
                empty: false,
                source: url,
                template: template || '{{ name }} &nbsp;<span>{{ materialSn }} &nbsp;{{ specification }}</span>',
                callback: function(value, index, selected) {
                    callback && callback(value, index, selected, this);
                }
            });
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
            ...
            var ajaxOptions = {
                ...
                success: function(data) {
                    ...
                    if(data.status === 0){
                        data = data.data;
                        //入库，出库查询列表要求显示总金额
                        if(p.hasTotalBar) { p.hasTotalBar(data); }
                    }else{
        ...
        _createEditor: function(editorBuilder, container, editParm, width, height) {
            ...
            //modified s
            var self = this;
            $('input', self.element).on('keydown', function (e) {
                if(e.keyCode === 13) {//回车
                    self.endEditToNext();
                }
                if(e.keyCode === 9) {//tab
        ...
        beginEdit: function(rowParm, containerBulider) {
            ...
            for (var i = 0, l = g.columns.length; i < l; i++) {
                ...
                var width = container.width(),
                    //height = container.height(); //modified
                    height = 30; //这里写死
        ...
        */
        ...
        _getCellHtml: function(rowdata, column) {
            ...
            //htmlarr.push('<div');
            //modified s //给列增加class属性
            //htmlarr.push('<div class="l-grid-row-cell-inner"');
            htmlarr.push('<div class="l-grid-row-cell-inner ' + column.columnname + '"');
            //modified e
            // 不同浏览器宽度显示 start @liufei
            var browser = navigator.userAgent.toLowerCase();
            if (browser.indexOf('firefox') > 0) { // 火狐
                htmlarr.push(' style = "width:' + parseInt(column._width - 6) + 'px;');
            } else {
                htmlarr.push(' style = "width:' + parseInt(column._width - 8) + 'px;');
            }
            // 不同浏览器宽度显示 end @liufei
        ...
        _applyEditor: function(obj) {
            var g = this,
                p = this.options,
                cols = g._columnCount; // fixBug add @liufei
            var rowcell = obj,
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                g.gridbody.bind('scroll.grid', function() {
                    g.scrollLeft = g.gridbody.scrollLeft();
                    g.scrollTop = g.gridbody.scrollTop();

                    //modified s   编辑框存在时滑动滚动轴 输入框一起移动
                    if(!!container) {
                        var sL = g.scrollLeft ? g.scrollLeft : 0;
                        var sT = g.scrollTop ? g.scrollTop : 0;
                        var l = t = 0;

                        l = pc.left + pb.left + pv.left - sL;
                        t = pc.top + pb.top + pv.top + topbarHeight - sT;

                        l += p.editorLeftDiff || 0;
                        t += p.editorTopDiff || 0;
                        container.css({left: l, top: t-1}); //下面的浏览器 left，top，height，暂不做处理
                    }
                    //modified e

                });
                var scrollLeft = g.scrollLeft ? g.scrollLeft : 0;
                var scrollTop = g.scrollTop ? g.scrollTop : 0;
                left = pc.left + pb.left + pv.left - scrollLeft;
                top = pc.top + pb.top + pv.top + topbarHeight - scrollTop;
                // bug 修复结束
                ...
                g.bind('sysEndEdit', function() {
                    // fixBug add @liufei l-grid-row-cell-editing
                    var idLastNum = editParm.column['__id'];
                    if (idLastNum.substr('2').indexOf(cols) > '-1') {
                        var tr = !!$(obj).parents('.l-grid-row').length ? $(obj).parents('.l-grid-row').next('.l-grid-row') : '';
                        if (!!tr.length) {
                            var rowId = tr.attr('id').split('|');
                            for (var rowid in g.records) {
                                if (rowid == rowId[rowId.length - 1]) {
        ...
        _getTotalCellContent: function(column, data) {
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    var precision = column.totalSummary.precision || 2;

                    if (isExist('sum'))
                        totalsummaryArr.push("<div>合计：" + info.sum.toFixed(precision) + "</div>");
                    if (isExist('tsum'))
        ...
        _getTotalSummaryHtml: function(data, classCssName, frozen) {
            ...
            $(g.columns).each(function(columnindex, column) {
                ...
                if (column._hide)
                    totalsummaryArr.push('style="display: none;"');
        ...
        _setEvent: function() {
            ...
            //表体 - 滚动联动事件
            g.gridbody.bind('scroll.grid', function() {
                var scrollLeft = g.scrollLeft = g.gridbody.scrollLeft();
                var scrollTop = g.scrollTop = g.gridbody.scrollTop();
                if (scrollLeft != null)

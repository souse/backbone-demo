    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        toggleSwitch: function(btn) {
            ...
            $('body').on('click', function(e) {
                var $target = $(e.target), 
                    $parents = $target.parents('.l-tab-windowsswitch');
        ...
        _addTabItemEvent: function(tabitem) {
            ...
            $(".l-tab-links-item-close", tabitem).hover(function() {
                ...
                //modified s //点击删除按钮 删除tab时 判断是否是编辑页面
                var arr = ['reimbursement-createreim', 'purchase-createpurchase', 'purchase-createinvoice', 'purchase-createbalance', 'warehouse-createstorein', 'warehouse-createderstorein'
                    ,'warehouse-createstoreout', 'warehouse-createrefund', 'warehouse-createdepinventory', 'warehouse-createloss', 'warehouse-backstorein', 'warehouse-purchasestorein'];
                
                if(arr.indexOf(tabid) != -1) {
                    var dialog = new artDialog({
                        width: 200,
                        title: '提示信息',
                        content: '页面正处于编辑状态，关闭后将丢失数据！',
                        class: 'md-tips',
                        button: [{
                                value: '确定',
                                callback: function() {

...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (p.absolute)
                g.selectBox.appendTo('#maingrid').addClass("l-box-select-absolute");
            else
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p && p.absolute)
            {
            			...
            			&& contentHeight > Number(g.selectBox.height() + 1))
                {
                    ...
                    var top = g.wrapper.offset().top - 60;
                    g.selectBox.css({ left: g.wrapper.offset().left - 11, top: top - 1 - g.selectBox.height() + (p.selectBoxPosYDiff || 0) });
                } else
                {
                    //modified s
                    //g.selectBox.css({ left: g.wrapper.offset().left -11, top: g.wrapper.offset().top + 1 + g.wrapper.outerHeight() + (p.selectBoxPosYDiff || 0) - 60 });
                    g.selectBox.css({ left: g.wrapper.offset().left - 11, top: g.wrapper.offsetParent().position().top + 32 });
                    //modified e
                }
                if (p.alwayShowInDown)
                {
                    g.selectBox.css({ left: g.wrapper.offset().left -11, top: g.wrapper.offset().top + 1 + g.wrapper.outerHeight() - 60 });
                }
            }
            else
            {
